  ...
  if (isPremium) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] p-6 text-center animate-fade-in">
        <div className="flex h-20 w-20 items-center justify-center rounded-3xl bg-accent/10 ring-1 ring-accent/25 mb-6 shadow-[0_0_40px_-5px_var(--accent-glow)]">
          <Crown size={36} className="text-accent" strokeWidth={2.5} />
        </div>
        <h1 className="text-3xl lg:text-4xl font-extrabold text-foreground font-[family-name:var(--font-outfit)] tracking-tight mb-3">
  ...
  return (
    <div className="flex flex-col items-center justify-center px-4 py-12 lg:py-16 min-h-screen relative">
      ...
      <div className="text-center mb-12 animate-fade-in relative z-10 w-full">
        <div className="inline-flex h-14 w-14 items-center justify-center rounded-2xl bg-accent/10 ring-1 ring-accent/25 mb-6 shadow-[0_0_20px_-5px_var(--accent-glow)]">
          <Crown size={24} className="text-accent" strokeWidth={2.5} />
        </div>
        <h1 className="text-4xl lg:text-5xl font-extrabold text-foreground font-[family-name:var(--font-outfit)] tracking-tight mb-4 leading-tight">
          ...
          return (
            <div
              ...
              className={`glass-card rounded-3xl p-6 lg:p-7 flex flex-col relative transition-all duration-300 hover:-translate-y-1 ${isRecommended
                ? 'border-accent bg-accent/[0.04] ring-2 ring-accent/30 shadow-[0_0_40px_-10px_var(--accent-glow)]'
                : 'hover:border-accent/30'
                }`}
            >
              {isRecommended && (
                <span className="absolute -top-3 left-1/2 -translate-x-1/2 bg-accent text-background text-[10px] font-extrabold px-4 py-1.5 rounded-full uppercase tracking-widest shadow-lg">
                  Recomendado
                </span>
              )}

              <h3 className="text-xl font-bold text-foreground font-[family-name:var(--font-outfit)] mb-1.5">{plan.label}</h3>
              ...
              <div className="mb-6 mt-1">
                <div className="flex items-baseline gap-1.5">
                  <span className="text-3xl lg:text-4xl font-extrabold text-foreground tracking-tight font-[family-name:var(--font-outfit)]">
                    {formatBRL(plan.price)}
                  </span>
                </div>
                {plan.plan_months > 1 ? (
                  <p className="text-[13px] text-muted-secondary mt-1.5 font-medium flex items-center gap-1.5">
                    Equivale a <strong className="text-muted-medium">{formatBRL(plan.price_per_month)}/mês</strong>
                  </p>
                ) : (
                  <p className="text-[13px] text-muted-secondary mt-1.5 font-medium flex items-center gap-1.5">
                    Cobrado mensalmente
              ...
              <ul className="text-[14px] text-muted space-y-3.5 mb-8 flex-1">
              ...
              <button
                ...
                className={`w-full py-4 rounded-xl font-extrabold text-[14px] transition-all duration-200 uppercase tracking-wider disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 ${isRecommended
                  ...
                  : 'bg-card-hover text-foreground hover:bg-foreground hover:text-background ring-1 ring-border border-none'
                  }`}
              >
      ...
      <div className="w-full max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-center gap-6 pt-6 border-t border-border/40 animate-fade-in stagger-2 relative z-10">
        <div className="flex flex-wrap items-center justify-center md:justify-end gap-x-6 gap-y-3">
          <div className="flex items-center gap-1.5">
            <ShieldCheck size={14} className="text-muted-faint" />
            <span className="text-[10px] text-muted-tertiary font-bold uppercase tracking-wider">Pagamento Seguro</span>
          </div>
          <div className="flex items-center gap-1.5">
            <Zap size={14} className="text-muted-faint" />
            <span className="text-[10px] text-muted-tertiary font-bold uppercase tracking-wider">Acesso Imediato</span>
          </div>
          <div className="flex items-center gap-1.5">
            <CalendarDays size={14} className="text-muted-faint" />
            <span className="text-[10px] text-muted-tertiary font-bold uppercase tracking-wider">Cancelamento Flexível</span>
          </div>
        </div>
      </div>
